...
function promodise_menus() {
	...
	$locations = array(
		...
		'footer_links' => __('Footer Links', 'promodise'),
	);
...
add_action('widgets_init', 'promodise_widgets_init');

function promodise_widgets_init() {
	register_sidebar(array(
		'name'          => esc_html__('Сайдбар блога', 'promodise'),
		'id'            => "sidebar-blog",
...
class Bootstart_Walker_Comment extends Walker {
...
add_action('init', 'my_custom_posts_init');
function my_custom_posts_init(){
	...
	register_post_type('partners', array(
		'labels'             => array(
			...
			'not_found'          => __('Партнера не найдено'),
	...
	register_post_type('prices', array(
		'labels'             => array(
			...
			'add_new'            => __('Добавить новую'),
			'add_new_item'       => __('Добавить новую цену'),
			'edit_item'          => __('Редактировать цену'),
			'new_item'           => __('Новая цена'),
			'view_item'          => __('Посмотреть цену'),
			'search_items'       => __('Найти цену'),
			'not_found'          => __('Цен не найдено'),
			'not_found_in_trash' => __('В корзине цен не найдено'),
			'parent_item_colon'  => '',
			'menu_name'          => __('Цены')
...
function my_action_callback() {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$headers = "От: Wordpress <info@promodise.com>";
		...
		// Если запрос не является POST-запросом, просто выход
